"""Interpolation functions for LCHuv color space."""

from colormath.color_objects import LCHuvColor, sRGBColor
from colormath.color_conversions import convert_color


def interpolate_lchuv_percentage(color_start, color_end, percentage):
    """Takes two LCHuv colors and a percentage, returns an RGB color."""
    ...
    # convert back to RGB
    lchuv = LCHuvColor(new_l, new_c, new_h)
...
def interpolate_lchuv(color_start, color_end, steps):
    ...
    # convert to LCHuv
    lchuv_start = convert_color(color_start, LCHuvColor)
    lchuv_end = convert_color(color_end, LCHuvColor)

    # edge case, steps = 1
    if steps == 1:
        return [interpolate_lchuv_percentage(lchuv_start, lchuv_end, 0.5)]
    ...
    for _ in range(steps):
        colors.append(interpolate_lchuv_percentage(
